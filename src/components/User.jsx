import { CardSkeleton, UserDetail } from '../components'
import { useAppGlobalContext } from '../context'
import { useUser } from '../hooks/'

function User () {
  const { searchedName, formatDate } = useAppGlobalContext()
  const { data, isPending, error } = useUser(searchedName)
  ...
  return (
    <article className='user-card br-medium'>
      <header className='user-card__header profile-card'>
        ...
        <p className='profile-card__name'>@{login}</p>
        <p className='profile-card__date'>
          Joined{' '}
          <span>{joinDate ? formatDate(joinDate) : `Not Available`}</span>
        </p>
        {bio ? (
      ...
      <dl className='user-card__links user-links'>
        ...
        <UserDetail
          ...
          label='company'
          content={company}
          // href={`https://github.com/${company.substring(1)}`}
        ></UserDetail>
      </dl>
    </article>
  )
